...
async function createSource() {
  ...
  else if (platform === 'taptap') { payload.baseUrl = $('#cfgBaseUrl').value.trim(); if (!payload.baseUrl) return toast('请填写网页地址'); }
...
async function saveSource(source) {
  ...
  if (normalizedPlatform(source) === 'bigplayer_h5') {
    ...
    patch.baseUrl = $('#cfgBaseUrl').value.trim(); if (!patch.baseUrl) return toast('请填写网页地址');
  }
