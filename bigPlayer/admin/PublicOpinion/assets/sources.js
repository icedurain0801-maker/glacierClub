...
function facebookForm(source, creating) {
  ...
  return `<form id="facebookSourceForm" class="facebook-source-form" novalidate><div class="drawer-body">
    ...
    <section class="detail-block"><h3 class="detail-label">基础信息</h3>
      ...
      <div class="field"><label>启用状态</label><label class="toggle"><input type="checkbox" id="cfgEnabled" ${source.enabled && !creating ? 'checked' : ''} ${creating || !writable || !ready ? 'disabled' : ''}><span class="track"></span><span id="facebookEnableHint">${creating ? '新建来源默认停用' : ready ? '启用并保存后进入同步调度' : '部署级凭据与六项官方能力全部通过后可启用'}</span></label></div>
...
function bindFacebookForm(source, creating) {
  ...
  const update = () => {
    ...
    $('#cfgEnabled').disabled = creating || !facebookCanManage(source) || !facebookReady(source) || changed;
    if (changed) $('#cfgEnabled').checked = false;
    if (!creating) {
...
function facebookFormPayload(source, creating) {
  ...
  const payload = { displayName, baseUrl: parsed.url, frequencySeconds, syncMode: initial === 'incremental' ? 'incremental' : 'backfill', historyStart: initial === 'all' ? FACEBOOK_ALL_HISTORY_START : initial === 'since' ? `${date}T00:00:00+08:00` : '', enabled: !creating && !changed && facebookReady(source) && Boolean($('#cfgEnabled').checked) };
...
function bindH5CredentialMode(source, creating) {
  const switchMode = mode => { state.h5AuthMode = mode; const baseUrl = $('#cfgBaseUrl')?.value || ''; const name = $('#cfgName')?.value || ''; const frequency = $('#cfgFreq')?.value || ''; const syncMode = $('#cfgSyncMode')?.value || 'incremental'; const historyStart = $('#cfgHistoryStart')?.value || ''; source.base_url = baseUrl; source.display_name = name; source.frequency_seconds = Number(frequency || 3600); source.sync_mode = syncMode; source.history_start = historyStart; renderDetail(source); };
...
function commonFields(source, creating) {
  const platform = normalizedPlatform(source); const available = ACTIONABLE.has(platform); const enabledAllowed = available && (canSchedule(source) || source.enabled);
  const selected = state.scope?.selected?.() || {};
  return `<div class="detail-block"><div class="detail-label">基础配置</div>${creating ? `<div class="field"><label>归属范围</label><input class="input" value="${esc([selected.regionLabel, selected.communityLabel].join(' / '))}" readonly></div><input type="hidden" id="cfgCommunity" value="${esc(selected.communityId)}"><input type="hidden" id="cfgPlatform" value="${esc(platform)}"><div class="field"><label>平台</label><input class="input" value="${esc(platformLabel(platform))}" readonly></div>` : ''}<div class="field"><label>采集源名称<span class="required">*</span></label><input class="input" id="cfgName" value="${esc(source.display_name || source.displayName || '')}"></div><div class="field"><label>启用</label><label class="toggle"><input type="checkbox" id="cfgEnabled" ${source.enabled ? 'checked' : ''} ${creating || !enabledAllowed ? 'disabled' : ''}><span class="track"></span><span>${creating ? '创建后固定关闭' : available ? (enabledAllowed ? '进入同步调度' : '等待登录与帖子能力可用') : '连接器不可用'}</span></label></div><div class="field"><label>${isTaptapSource(source) ? '抓取频率' : '采集频率'}</label>${frequencySelect(source)}</div>${isTaptapSource(source) ? scheduleTimeField(source) : ''}</div>`;
}
...
async function saveSource(source) {
  ...
  const selectedMode = $('#cfgSyncMode')?.value || sourceMode(source); const historyStart = $('#cfgHistoryStart')?.value || '';
  const patch = { displayName: $('#cfgName').value.trim(), enabled: $('#cfgEnabled').checked, frequencySeconds: Number($('#cfgFreq').value), syncMode: selectedMode, historyStart }; if (normalizedPlatform(source) === 'taptap') patch.scheduleTime = patch.frequencySeconds === 86400 ? ($('#cfgScheduleTime')?.value || '03:00') : null;
  let credential;
  if (!patch.displayName) return toast('请填写采集源名称'); if (!Number.isInteger(patch.frequencySeconds) || patch.frequencySeconds <= 0) return toast('请选择有效的采集频率'); if (patch.syncMode === 'backfill' && !patch.historyStart) return toast('历史回溯必须填写历史起点');
