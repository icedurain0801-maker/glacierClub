...
function buildDeps(env = process.env) {
  return { repo: new Repository(), translator: new AiTranslator(env), version: env.AI_TRANSLATION_VERSION || 'translation-v1', batchSize: Number(env.AI_TRANSLATION_JOB_BATCH_SIZE || 20), backfillBatchSize: Number(env.AI_TRANSLATION_BACKFILL_BATCH_SIZE || 100), maxAttempts: Number(env.AI_TRANSLATION_JOB_MAX_ATTEMPTS || 3), retryBaseSeconds: Number(env.AI_TRANSLATION_JOB_RETRY_BASE_SECONDS || 60), leaseOwner: `translation-${process.pid}-${crypto.randomUUID()}` };
}
...
async function processJob(deps, job) {
  try {
    ...
    const terminal = Boolean(error?.noRetry) || Number(job.attempts || 0) >= deps.maxAttempts;
    const finished = await deps.repo.finishTranslationJob(job.id, { leaseOwner: job.lease_owner, status: terminal ? 'failed' : 'retryable', errorCode: errorCode(error), errorMessage: error?.message, retryAt: terminal ? null : retryAt(job.attempts, deps.retryBaseSeconds) });
