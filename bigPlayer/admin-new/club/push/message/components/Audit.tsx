...
import { auditPushMessage } from '@/api/club';

import { PUSH_STATUS_TYPE, PushAuditOptions, PushMessageItem, CLUB_DEPLOY_VERSION } from '@ts/club';

/* 批量审核 */

interface PushMessageAuditProps {
...
function PushMessageAudit(props: PushMessageAuditProps) {
    ...
    const handleOk = async () => {
        try {
            ...
            const { code, msg } = await auditPushMessage(
                {
        ...
        >
            <Spin spinning={batchAuditLoading}>
                <p className="batch-audit-tip">
                    <span>共审核</span> <span className="color-blue">{data.length}</span> <span>条消息</span>
                </p>
                <Form name="batchAuditForm" form={batchAuditForm} initialValues={{ status: PUSH_STATUS_TYPE.Approved }}>
                    <Form.Item name="status" label="审核结果" required>
                        <Radio.Group options={PushAuditOptions}></Radio.Group>
                    </Form.Item>
                    <Form.Item
                        name="remark"
                        label="审核备注"
...
export default PushMessageAudit;
